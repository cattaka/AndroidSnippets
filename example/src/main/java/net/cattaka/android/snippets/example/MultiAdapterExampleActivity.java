    ...
    ListenerRelay<SingleViewAdapter2, SingleViewAdapter2.ViewHolder> mHeaderListenerRelay = new ListenerRelay<SingleViewAdapter2, SingleViewAdapter2.ViewHolder>() {
        ...
        public void onClick(RecyclerView recyclerView, SingleViewAdapter2 adapter, SingleViewAdapter2.ViewHolder viewHolder, View view) {
            if (recyclerView.getId() == R.id.recycler) {
                ...
                if (la.mAdapter == mStringsHeaderAdapter) {
                    Tracker.getInstance().recordAction(me, TrackEvent.ACTION_CLICK, toParamsMap(TrackParam.ITEM_NAME, "string", TrackParam.ITEM_CATEGORY, "header"));
                    Snackbar.make(view, "Strings Header is clicked.", Snackbar.LENGTH_SHORT).show();
                } else if (la.mAdapter == mNumbersHeaderAdapter) {
                    Tracker.getInstance().recordAction(me, TrackEvent.ACTION_CLICK, toParamsMap(TrackParam.ITEM_NAME, "number", TrackParam.ITEM_CATEGORY, "header"));
        ...
        public boolean onLongClick(RecyclerView recyclerView, SingleViewAdapter2 adapter, SingleViewAdapter2.ViewHolder viewHolder, View view) {
            if (recyclerView.getId() == R.id.recycler) {
                MergeRecyclerAdapter.LocalAdapter la = mMergeRecyclerAdapter.getAdapterOffsetForItem(viewHolder.getAdapterPosition());
                if (la.mAdapter == mStringsHeaderAdapter) {
                    Tracker.getInstance().recordAction(me, TrackEvent.ACTION_LONG_CLICK, toParamsMap(TrackParam.ITEM_NAME, "string", TrackParam.ITEM_CATEGORY, "header"));
                    Snackbar.make(view, "Strings Header is long clicked.", Snackbar.LENGTH_SHORT).show();
                } else if (la.mAdapter == mNumbersHeaderAdapter) {
                    Tracker.getInstance().recordAction(me, TrackEvent.ACTION_LONG_CLICK, toParamsMap(TrackParam.ITEM_NAME, "number", TrackParam.ITEM_CATEGORY, "header"));
